...
//    public $email = "navvi84@gmail.com";

      public function mailinglist() {

        $mail = DB::table('mailinglist')->select('name', 'email')->get();
        return view('users/mailinglist', ['mail' => $mail]);
    }
    ...
    public function maillistsubmit() {
        ...
        for ($x = 0; $x < $value; $x++) {
            if (Input::get('mail_' . $x)) {
               ...
               $val= Mail::raw($message, function ($message)use($email) {
                    ...
                    $message->from('naveen.goud@karmanya.co.in', 'Naveen');
        ...
        $mail = DB::table('mailinglist')->select('name', 'email')->get();
        return view('users/mailinglist', ['mail' => $mail,'message'=>$messagereturn]);
    }
