...
class WebsiteRateController extends Controller {
    ...
    public function index() {
        ...
        if (count($article) == 0) {
            ...
            return View('users/WebsiteRate', ['Result' => $article, 'limit' => $limit]);
        }
    }
    ...
    public function get_articles($limit) {
        ...
        $limit = (int) $limit;
        $article = array();
        $data = DB::table('website')->select('Id', 'Title', 'RateTotal', 'RateCount')->take($limit)->get();
    ...
    public function rate($table, $item, $rating) {
        $item = (int) $item;
        $rating = (int) $rating;
        $rating_result = DB::table('website')->where('id', $item)->increment('RateCount', 1, ['RateTotal' => $rating]);
